<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Personnel Management') }}</h2>
    </x-slot>
    <div class="py-12">
                    ...
                    <div x-data="personnelApp" class="space-y-6 max-w-7xl mx-auto px-1" aria-label="Personnel management">
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                            <div class="bg-white border border-slate-200 rounded-xl px-5 py-4 shadow-sm hover:shadow-md transition-all">
                                ...
                                <p class="text-2xl font-black text-slate-900 leading-none" x-text="totalPersonnel"></p>
                            ...
                            <div class="bg-white border border-green-200 rounded-xl px-5 py-4 shadow-sm hover:shadow-md transition-all">
                                ...
                                <p class="text-2xl font-black text-green-600 leading-none" x-text="cleanCount"></p>
                            ...
                            <div class="bg-white border border-red-200 rounded-xl px-5 py-4 shadow-sm hover:shadow-md transition-all">
                                ...
                                <p class="text-2xl font-black text-red-600 leading-none" x-text="issueCount"></p>
                            ...
                            <div class="bg-white border border-blue-200 rounded-xl px-5 py-4 shadow-sm hover:shadow-md transition-all">
                                ...
                                <p class="text-2xl font-black text-blue-600 leading-none" x-text="fmtHours(totalHours)"></p>
                        ...
                        <div class="flex flex-wrap items-center justify-between gap-3 bg-white border border-slate-200 rounded-xl px-5 py-3 shadow-sm">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <button @click="toggleSort('name')" class="flex items-center gap-1 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all border" :class="sortBy === 'name' ? 'bg-blue-50 text-blue-600 border-blue-200' : 'bg-white border-slate-200 text-slate-500 hover:border-slate-300 hover:text-slate-700'">Name <span x-text="sortBy === 'name' ? (sortDirection === 'asc' ? '↑' : '↓') : ''"></span></button>
                                            ...
                                            <div class="bg-white border shadow-sm rounded-2xl overflow-hidden hover:shadow-md transition-all" :class="p.invalidRecordCount > 0 ? 'border-red-200' : 'border-slate-200'">
                                                <div class="p-5">
                                                    <div class="flex items-start gap-3 mb-4">
                                                        <div class="w-10 h-10 rounded-xl flex items-center justify-center font-black border" :class="p.invalidRecordCount > 0 ? 'bg-amber-100 text-amber-700 border-amber-200' : 'bg-green-100 text-green-700 border-green-200'" x-text="getInitials(p.fullName)"></div>
                                                    ...
                                                    <div class="mb-4">
                                                        <template x-if="p.invalidRecordCount > 0">
                                                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-red-50 text-red-600 border border-red-200 text-[10px] font-black uppercase tracking-wider" x-text="p.invalidRecordCount + ' Issue' + (p.invalidRecordCount !== 1 ? 's' : '')"></span>
                                                        </template>
                                                        <template x-if="p.invalidRecordCount === 0">
                                                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-green-50 text-green-600 border border-green-200 text-[10px] font-black uppercase tracking-wider">Clean</span>
                                                        </template>
                                                    </div>
                                                    <div class="grid grid-cols-3 gap-2 mb-4">
                                                        <div class="text-center p-2 bg-slate-50 rounded-lg">
                                                            <p class="text-xs font-black text-slate-900" x-text="p.sessionCount"></p>
                                                            <p class="text-[9px] text-slate-500 font-medium">Sessions</p>
                                                        </div>
                                                        <div class="text-center p-2 bg-slate-50 rounded-lg">
                                                            <p class="text-xs font-black text-slate-900" x-text="fmtHours(p.totalRegularMinutes)"></p>
                                                            <p class="text-[9px] text-slate-500 font-medium">Regular</p>
                                                        </div>
                                                        <div class="text-center p-2 bg-slate-50 rounded-lg">
                                                            <p class="text-xs font-black text-slate-900" x-text="fmtHours(p.totalOvertimeMinutes)"></p>
                                                            <p class="text-[9px] text-slate-500 font-medium">OT</p>
                                                        </div>
                                                    </div>
                                                    <div class="flex gap-2">
                                                        <button @click="viewHistory(p.fullName)" class="flex-1 py-2 bg-slate-50 hover:bg-blue-50 border border-slate-200 hover:border-blue-200 rounded-lg text-[10px] font-black text-slate-600 hover:text-blue-600 uppercase tracking-wider transition-all">View History</button>
                                                    </div>
                                                </div>
                                            </div>
                                        </template>
                                                ...
                                                <div class="p-5 sm:p-6">
                                                    <div class="flex flex-col lg:flex-row lg:items-center gap-5">
                                                        <div class="flex items-center gap-4 lg:min-w-[280px]">
                                                            <div class="w-12 h-12 rounded-xl flex items-center justify-center font-black border" :class="p.invalidRecordCount > 0 ? 'bg-amber-100 text-amber-700 border-amber-200' : 'bg-green-100 text-green-700 border-green-200'" x-text="getInitials(p.fullName)"></div>
                                                            <div class="min-w-0 flex-1">
                                                                <h3 class="text-base font-black text-slate-900 leading-tight truncate" x-text="p.fullName"></h3>
                                                        ...
                                                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 flex-1">
                                                            <div class="flex flex-col gap-2 p-3 bg-slate-50 border border-slate-200 rounded-xl hover:shadow-md transition-all">
                                                                ...
                                                                <p class="text-base font-black text-slate-900 leading-none" x-text="fmtHours(p.totalRegularMinutes)"></p>
                                                            ...
                                                            <div class="flex flex-col gap-2 p-3 bg-orange-50 border border-orange-200 rounded-xl hover:shadow-md transition-all">
                                                                ...
                                                                <p class="text-base font-black text-slate-900 leading-none" x-text="fmtHours(p.totalOvertimeMinutes)"></p>
                                                            ...
                                                            <div class="flex flex-col gap-2 p-3 rounded-xl hover:shadow-md transition-all" :class="p.totalUndertimeMinutes > 0 ? 'bg-red-50 border border-red-200' : 'bg-slate-50 border border-slate-200'">
                                                                <div class="flex items-center justify-between">
                                                                    <span class="text-[9px] font-black uppercase tracking-widest" :class="p.totalUndertimeMinutes > 0 ? 'text-red-700' : 'text-slate-600'">Undertime</span>
                                                                ...
                                                                <p class="text-base font-black text-slate-900 leading-none" x-text="fmtHours(p.totalUndertimeMinutes)"></p>
                            ...
                            <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" @click.self="closeHistory()">
                                <div class="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[80vh] overflow-y-auto p-8">
                                    <div class="flex items-center justify-between mb-6">
                                        <h3 class="text-xl font-black text-gray-900" x-text="'History: ' + selectedPersonnelName"></h3>
                                        <button @click="closeHistory()" class="p-2 rounded-lg hover:bg-gray-100 transition">
                                            <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                                        </button>
                                    </div>
                                    <template x-if="historySessions.length === 0">
                                        <p class="text-center py-8 text-gray-500">No session history found.</p>
                                    </template>
                                    <template x-if="historySessions.length > 0">
                                        <table class="min-w-full divide-y divide-gray-200">
                                            <thead class="bg-gray-50">
                                                <tr>
                                                    <th class="px-4 py-2 text-left text-xs font-black text-gray-500 uppercase">Date</th>
                                                    <th class="px-4 py-2 text-left text-xs font-black text-gray-500 uppercase">Time In</th>
                                                    <th class="px-4 py-2 text-left text-xs font-black text-gray-500 uppercase">Time Out</th>
                                                    <th class="px-4 py-2 text-left text-xs font-black text-gray-500 uppercase">Duration</th>
                                                    <th class="px-4 py-2 text-left text-xs font-black text-gray-500 uppercase">Status</th>
                                                </tr>
                                            </thead>
                                            <tbody class="bg-white divide-y divide-gray-200">
                                                <template x-for="s in historySessions" :key="s.id">
                                                    <tr>
                                                        ...
                                                        <td class="px-4 py-2">
                                                            <span class="px-2 py-0.5 rounded-full text-[10px] font-black" :class="s.status === 'COMPLETE' ? 'bg-green-100 text-green-800' : s.status === 'ONGOING' ? 'bg-blue-100 text-blue-800' : 'bg-red-100 text-red-800'" x-text="s.status"></span>
                                                        </td>
                                                    </tr>
                                                </template>
                                            </tbody>
                                    ...
                                    <div class="mt-4 flex justify-end">
                                        <button @click="closeHistory()" class="px-6 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold rounded-xl transition">Close</button>
                                    </div>
                                </div>
                            </div>
                        </template>
