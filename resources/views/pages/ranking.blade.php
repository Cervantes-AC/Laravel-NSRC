            ...
            <div x-data="rankingsApp" class="space-y-6" aria-label="{{ __('Rankings') }}">
                <div class="relative overflow-hidden rounded-3xl bg-gradient-to-br from-amber-500 via-orange-500 to-red-500 p-8 shadow-lg">
                    ...
                    <div class="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <h1 class="text-2xl font-bold text-white">{{ __('Volunteer Rankings') }}</h1>
                ...
                <template x-if="showScoringGuide">
                    <div class="bg-white border border-amber-200 rounded-2xl p-6 shadow-sm">
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="text-base font-bold text-gray-900">{{ __('How Scoring Works') }}</h3>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                            <div class="p-4 bg-green-50 border border-green-200 rounded-xl">
                                ...
                                <p class="text-sm font-bold text-green-800">Beginner</p>
                                <p class="text-xs text-green-600">&lt; 10 hours</p>
                            </div>
                            <div class="p-4 bg-blue-50 border border-blue-200 rounded-xl">
                                <div class="text-2xl mb-1">💪</div>
                                <p class="text-sm font-bold text-blue-800">Rising</p>
                                <p class="text-xs text-blue-600">10 – 24 hours</p>
                            </div>
                            <div class="p-4 bg-orange-50 border border-orange-200 rounded-xl">
                                <div class="text-2xl mb-1">🔥</div>
                                <p class="text-sm font-bold text-orange-800">Dedicated</p>
                                <p class="text-xs text-orange-600">25 – 49 hours</p>
                            </div>
                            <div class="p-4 bg-amber-50 border border-amber-200 rounded-xl">
                                <div class="text-2xl mb-1">⭐</div>
                                <p class="text-sm font-bold text-amber-800">Veteran</p>
                        ...
                        <div class="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-xl text-center">
                            ...
                            <p class="text-sm font-bold text-yellow-800">Century Club</p>
                            ...
                            <template x-if="topThree[entry]">
                                <div class="relative bg-white border border-gray-200 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all text-center" :class="entry === 0 ? 'sm:mb-0 ring-2 ring-yellow-400 ring-offset-2' : (entry === 1 ? 'sm:mb-4' : 'sm:mb-8')">
                                    <div class="absolute -top-3 left-1/2 -translate-x-1/2">
                                        <span class="inline-flex items-center justify-center w-8 h-8 rounded-full text-sm font-bold shadow-lg" :class="entry === 0 ? 'bg-yellow-400 text-yellow-900' : (entry === 1 ? 'bg-gray-300 text-gray-700' : 'bg-orange-300 text-orange-900')" x-text="entry === 0 ? '1st' : (entry === 1 ? '2nd' : '3rd')"></span>
                                    </div>
                                    <div class="w-16 h-16 mx-auto rounded-2xl flex items-center justify-center font-bold text-xl border-2 shadow-sm mt-2" :class="entry === 0 ? 'bg-yellow-100 text-yellow-800 border-yellow-300' : (entry === 1 ? 'bg-gray-100 text-gray-700 border-gray-300' : 'bg-orange-100 text-orange-800 border-orange-300')" x-text="(topThree[entry].full_name || '??').substring(0, 2).toUpperCase()"></div>
                                    <h3 class="mt-3 text-base font-bold text-gray-900 truncate" x-text="topThree[entry].full_name || 'Unknown'"></h3>
                                    <p class="text-xs text-gray-500 font-medium" x-text="(topThree[entry].session_count || 0) + ' sessions'"></p>
                                    <div class="mt-3 grid grid-cols-2 gap-2">
                                        <div class="p-2 bg-gray-50 rounded-lg">
                                            <p class="text-sm font-bold text-gray-900" x-text="fmtHours(topThree[entry].total_minutes || 0)"></p>
                                            <p class="text-[10px] text-gray-500 font-medium uppercase tracking-wider">Total</p>
                                        </div>
                                        <div class="p-2 bg-gray-50 rounded-lg">
                                            <p class="text-sm font-bold text-gray-900" x-text="topThree[entry].session_count || 0"></p>
                                            <p class="text-[10px] text-gray-500 font-medium uppercase tracking-wider">Sessions</p>
                                        </div>
                                    </div>
                                    <div class="mt-3 inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider bg-gray-50 border border-gray-200">
                ...
                <div class="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
                    <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
                        <h3 class="text-base font-bold text-gray-900">{{ __('Leaderboard') }}</h3>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">{{ __('Rank') }}</th>
                                    <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">{{ __('Name') }}</th>
                                    <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">{{ __('Achievement') }}</th>
                                    <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">{{ __('Total Hours') }}</th>
                                    <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">{{ __('Regular') }}</th>
                                    <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">{{ __('Overtime') }}</th>
                                    <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">{{ __('Sessions') }}</th>
                                </tr>
                            </thead>
                                ...
                                <template x-for="(entry, idx) in rankings" :key="entry.id || entry.full_name">
                                    <tr :class="idx < 3 ? 'bg-amber-50/50' : 'hover:bg-gray-50'">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <template x-if="idx === 0"><span class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-yellow-100 text-yellow-800 font-bold text-sm">🥇</span></template>
                                            <template x-if="idx === 1"><span class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-gray-100 text-gray-800 font-bold text-sm">🥈</span></template>
                                            <template x-if="idx === 2"><span class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-orange-100 text-orange-800 font-bold text-sm">🥉</span></template>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center gap-3">
                                                <div class="w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold" :class="idx < 3 ? 'bg-amber-100 text-amber-800' : 'bg-gray-100 text-gray-700'" x-text="(entry.full_name || '??').substring(0, 2).toUpperCase()"></div>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <template x-for="ach in getAchievements(entry.total_minutes || 0)" :key="ach.label">
                                                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-white border border-gray-200" :class="ach.color"><span x-text="ach.icon"></span><span x-text="ach.label"></span></span>
                                            </template>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900" x-text="fmtHours(entry.total_minutes || 0)"></td>
